...
반복문을 통한 코드를 대신하여 콘솔출력(System.out.println)의 횟수를 줄여 View의 책임을 더욱 명확히 하기 위해서 이다. -> 즉 간단히 말하자면 List를 순회하며 String에 + rc.getName()을 하는 작업을 람다식(Stream API)으로 더 간결하고 고급스럽게 표현한 것!
